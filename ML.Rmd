...
It looks like the CV error is minimized for $k = 13$ and $size = 4$ 
...
Notice that only two predictors were used in the construction of this tree, but they are split 6 times. The *residual mean deviance* is just the mean squared error for the tree. `Accept` refers to the number of applications accepted. It makes perfect sense that this quantity would be correlated with `Apps` which is the number of applications received. I did not expect that `Top25perc`, which is the percentage of students in the top 25% of their high school class, would be the other important variable; however, it is only used in one split. 
...
rcv.college
```
It looks like the best trees are of size 1 and size 5. The function threw an error when I tried to make predictions using a single-node tree, so we will use the tree of size 5.

```{r}
rprune.college <- prune.tree(rtree.college, best = 5)
...
Noice that this performs worse than the whole tree. In fact, the performance decreases as the size of the tree decreases, so we will keep the full model. 
...
```{r}
my_results <- rbind(my_results, list("Boost", BOOST.RMSE)) %>% arrange(RMSE)
knitr::kable(my_results, digits = 3)
```
